...
for d in [RAW, CLEAN, REJECTED, ANALYSIS]:
...
with sync_playwright() as p:
    ...
    for it in items:
        ...
        clean_path = CLEAN / f'tweet_{tid}.png'
        raw_path = RAW / f'tweet_{tid}.png'
        rej_path = REJECTED / f'tweet_{tid}.png'

        if clean_path.exists() or rej_path.exists():
            skipped_existing += 1
            continue
        ...
        try:
            ...
            if article.count() == 0:
                page.screenshot(path=str(rej_path))
                row['mode'] = 'no-article'
                results.append(row)
            ...
            if ab:
                page.screenshot(
                    path=str(raw_path),
                    clip={'x': max(0, ab['x'] - 12), 'y': max(0, ab['y'] - 12), 'width': ab['width'] + 24, 'height': ab['height'] + 24},
                )
            ...
            if media:
                page.screenshot(
                    path=str(clean_path),
                    clip={'x': max(0, media['x'] - 10), 'y': max(0, media['y'] - 10), 'width': media['width'] + 20, 'height': media['height'] + 20},
                )
                row['status'] = 'ok'
                row['mode'] = 'media'
            elif ab:
                page.screenshot(
                    path=str(clean_path),
                    clip={'x': max(0, ab['x'] - 8), 'y': max(0, ab['y'] - 8), 'width': ab['width'] + 16, 'height': ab['height'] + 16},
                )
                row['status'] = 'ok'
                row['mode'] = 'article-fallback'
            else:
                page.screenshot(path=str(rej_path))
                row['mode'] = 'no-bbox'
...
summary = {
    ...
    'captured_now': len(results),
    'ok': sum(1 for r in results if r['status'] == 'ok'),
    'rejected': sum(1 for r in results if r['status'] != 'ok'),
    'results': results,
}
(ANALYSIS / 'capture_report_missing.json').write_text(json.dumps(summary, indent=2))
print('\nDone', summary)
